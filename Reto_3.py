...
def Llenado_Tarjetas(mat_base, n):
    ...
    carton_final = Reto_2.crear_matriz(n)
    idx = 0
    for i in range(n):
        for j in range(n):
            if mat_base[i, j] > 0:
...
def ejecutar_reto_3():
    ...
    mat_base = None
    if codigo == "A1": mat_base = Reto_2.generar_A1(n)
    elif codigo == "A2": mat_base = Reto_2.generar_A2(n)
    elif codigo == "B1": mat_base = Reto_2.generar_B1(n)
    elif codigo == "B2": mat_base = Reto_2.generar_B2(n)
    elif codigo == "C1": mat_base = Reto_2.generar_C1(n)
    elif codigo == "C2": mat_base = Reto_2.generar_C2(n)
    elif codigo == "D1": mat_base = Reto_2.generar_D1(n)
    elif codigo == "D2": mat_base = Reto_2.generar_D2(n)
    elif codigo == "E1": mat_base = Reto_2.generar_E1(n)
    elif codigo == "E2": mat_base = Reto_2.generar_E2(n)
    elif codigo == "F1": mat_base = Reto_2.generar_F1(n)
    elif codigo == "F2": mat_base = Reto_2.generar_F2(n)
    elif codigo == "G1": mat_base = Reto_2.generar_G1(n)
    elif codigo == "G2": mat_base = Reto_2.generar_G2(n)
    elif codigo == "H1": mat_base = Reto_2.generar_H1(n)
    elif codigo == "H2": mat_base = Reto_2.generar_H2(n)
    ...
    Reto_2.imprimir_tarjetas_formateadas(carton_final, mat_base, "CARTÓN GENERADO", nombre_jugador)
    input("Presione ENTER para comenzar el sorteo...")

    # 4. EL JUEGO (TOMbola)
    marcado = Reto_2.crear_matriz(n)
    bolillos_sorteados = []
    terminado = False
