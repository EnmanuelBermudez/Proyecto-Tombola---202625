...
def guardar_juego_binario(cedula, fecha_hora, secuencia_carton, bolillos_sorteados):
    ...
    with open(archivo, "ab") as f:
        ...
        # Escribir los números del cartón uno por uno como enteros (I)
        for num in secuencia_carton:
            f.write(struct.pack('I', num))
            
        # Escribir los bolillos sorteados uno por uno como enteros (I)
...
def ejecutar_reto_3():
    ...
    if os.path.exists("REG_JUGADORES.bin"):
        f = open("REG_JUGADORES.bin", "rb")
        while True:
    ...
    # 2. SELECCIÓN DE ODS (Uso de IF-ELIF en lugar de diccionarios)
    n = 0
    while n < 5 or n % 2 == 0:
    ...
    # 3. LLENADO DE TARJETA (Sin usar random.shuffle ni np.where)
    # Contar cuántas celdas tiene la figura
    celdas_figura = 0
    max_orden = 0
    for i in range(n):
        for j in range(n):
    ...
    # Asignar los números al cartón según el orden (1, 2, 3...)
    carton_final = reto2.crear_matriz(n)
    for k in range(1, max_orden + 1):
        for i in range(n):
            for j in range(n):
                if mat_base[i, j] == k:
                    # El índice k-1 corresponde al número generado
                    val = carton_numeros[k-1]
                    carton_final[i, j] = val
                    secuencia_para_archivo.append(val)
    ...
    while not terminado and len(bolillos_sorteados) < (n*n):
        # Generar bolillo único
        bolillo = random.randint(1, n*n)
        ya_salio = False
        for b in bolillos_sorteados:
            if b == bolillo: ya_salio = True
        
        if ya_salio: continue
        ...
        # Dibujar tablero con marcas (manual)
        for i in range(n):
            fila_str = ""
            for j in range(n):
                if mat_base[i, j] == 0:
                    ...
                    fila_str += "| [X] " # X significa marcado
                else:
                    fila_str += "| {:^3} ".format(carton_final[i, j])
            print(fila_str + "|")

        # Verificar victoria (manual)
        gano = True
        for i in range(n):
            for j in range(n):
                # Si la celda es parte de la figura pero no está marcada...
